...
class main_page(object):
    ...
    def page_designer(self):
        ...
        Button(self.main_page, text="查询当前配置信息", width=52, height=1,command=self.showinfo).grid(row=1,column=0,columnspan=3,pady=6)
        ...
        self.cmb['value']=('配置1','配置2','配置3','配置4','配置5','配置6','配置7','配置8','配置9','配置10','配置11','配置12','配置13','配置14','配置15')
        self.cmb.set(self.cmb['value'][0])
        Button(self.main_page,text='确认切换',width=13,height=1,command=self.switch_and_show).grid(row=2,column=2,padx=10,pady=20,sticky=W)
        self.txt=Text(self.main_page, width=54, height=33, background='#ffffff', borderwidth=3).grid(row=3,column=0,columnspan=3,padx=5,pady=6,sticky=W)
